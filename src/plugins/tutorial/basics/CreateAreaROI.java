 ...
 * 
 * \page tuto9 Tutorial: Creating an Area ROI from code.
 * 
 * \code
 * 
 * public class CreateAreaROI extends Plugin implements PluginImageAnalysis {
	...
	public void compute() {
		...
		for ( int i = 0 ; i < dataBuffer.length ; i++ )
		{
			mask[i] = ( dataBuffer[i] > mean ); // equivalent to if ( dataBuffer[i] > mean ) mask[i]=true; else mask[i]=false;
		}
